...
minmax(coarse_trace_clim$tasmax - coarse_trace_clim$tasmin) # some neg DTR values
...
sum(values(trace_dtr) <= 0, na.rm = TRUE)/(ncell(trace_dtr)*nlyr(trace_dtr)) * 100 # ~2%
...
tmin <- (tavg - 0.5*tdtr)
tmax-tmin
panel(tmax - tmin < 0, fun = function() lines(land))
