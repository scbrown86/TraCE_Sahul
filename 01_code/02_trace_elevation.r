...
landmask <- rast("/mnt/Data/TraCE21_Sahul/LANDFRAC/trace.01-36.22000BP.cam2.LANDFRAC.22000BP_decavg_400BCE.Sahul.preProc.nc", "LANDFRAC")[[2204]] #2204
landmask
...
plot(merge(
  mask(project(mask(elev_interp[[11]], landmask_fine), landmask, "average"), landmask),
  mask(project(mask(elev_interp[[11]], landmask_fine, inverse = TRUE), landmask, "average"),
    landmask,
    inverse = TRUE
  )), fun = function() lines(land)
)
...
writeCDF(elev_coarse, "./02_data/01_inputs/TraCE21_elevation_1450CE_1950CE.nc", varname = "elevation", longname = "elevation", unit = "m", compression = 1, overwrite = TRUE)
