...
brown <- list.files(
  path = "/media/dafcluster4/storage/TraCE_1500_1990CE/1500_1990/out",
  pattern = "biascorr.nc",
  recursive = TRUE,
  full.names = TRUE
)
...
chelsa_trace <- list(
  pr = rast(chelsa_fil[1:12], win = ext(land)),
...
dt_summary <- readRDS("03_comparisons/koppen_comparisons_contemporary.RDS")
...
step_summaries <- koppen_summary(list(brown, agcd, chelsa_baseline, chelsa_trace))
...
plot(
  x = step_summaries$brown[Koppen == "Temperate" & variable == "tas", ][["Year"]],
  y = step_summaries$brown[Koppen == "Temperate" & variable == "tas", ][["mean"]],
  type = "l", col = "#d95f02", ylim = c(12, 16)
)
lines(
  x = step_summaries$agcd[Koppen == "Temperate" & variable == "tas", ][["Year"]],
  y = step_summaries$agcd[Koppen == "Temperate" & variable == "tas", ][["mean"]],
  col = "#1b9e77"
)
lines(
  x = step_summaries$CHELSA[Koppen == "Temperate" & variable == "tas", ][["Year"]],
  y = step_summaries$CHELSA[Koppen == "Temperate" & variable == "tas", ][["mean"]],
  col = "#7570b3"
)

plot(
  x = step_summaries$brown[Koppen == "Temperate" & variable == "pr", ][["Year"]],
  y = step_summaries$brown[Koppen == "Temperate" & variable == "pr", ][["mean"]],
  type = "l", col = "#d95f02"
)
lines(
  x = step_summaries$agcd[Koppen == "Temperate" & variable == "pr", ][["Year"]],
  y = step_summaries$agcd[Koppen == "Temperate" & variable == "pr", ][["mean"]],
  col = "#1b9e77"
)
lines(
  x = step_summaries$CHELSA[Koppen == "Temperate" & variable == "pr", ][["Year"]],
...
vars <- c("mean", "SD")
step_summaries_roll[, paste0("roll_", vars) := frollmean(.SD,
  n = 10, align = "right",
  hasNA = FALSE
),
...
set(step_summaries_roll,
  i = which(step_summaries_roll[["MODEL"]] == "CHELSA_Trace"),
...
expanded <- CJ(
  Year = years, Koppen = unique_combos$Koppen,
...
merged[, `:=`(
  ...
  SD = SD[!is.na(SD)][1]
), by = .(Koppen, variable, MODEL)]


p1 <- ggplot(
  data = step_summaries_roll[MODEL %in% c("BROWN"), ], # , "AGCD"), ],
  aes(
    x = Year, y = roll_mean,
  ...
  scale_fill_manual(
    values = c("BROWN" = "#1b9e77", "AGCD" = "#d95f02", "CHELSA_Trace" = "#7570b3"),
...
pdf(
  file = "03_comparisons/zonal_means.pdf",
...
pdf(
  file = "03_comparisons/raster_means.pdf",
  paper = "a4", bg = "white",
  pagecentre = TRUE
)
{
  p6 <- plot(
    crop(mask(
      c(
        app(CHELSA_Trace21_1950$pr, sum),
  ...
  p7 <- plot(
    crop(mask(c(
      app(CHELSA_Trace21_1950$tas, mean),
  ...
  p8 <- plot(
    crop(mask(c(
      app(CHELSA_Trace21_1950$tasmin, mean),
  ...
  p9 <- plot(
    crop(mask(c(
      app(CHELSA_Trace21_1950$tasmax, mean),
...
qpdf::pdf_combine(
  input = c(
    "03_comparisons/zonal_means.pdf",
...
pdf(
  file = "03_comparisons/comparisons_delta.pdf",
...
comm <- c(
  agcd$pr[[1]], brown_1910_1989$pr[[1]], CHELSA_Trace21_1950$pr[[1]],
...
#### ZONAL TAYLOR ####
pdf(
  file = "03_comparisons/comparisons_taylor_koppen.pdf",
  width = 8, height = 8, onefile = TRUE, bg = "white"
)
par(mai = c(0.5, 0.5, 0.5, 0.5), mar = c(0.5, 0.5, 0.5, 0.5))
{
  {
    taylor_from_spatraster_zones(
      obs = CHELSA_Trace21_1950$tas,
      mod = brown_1910_1989$tas,
      zones = koppen,
      add = FALSE,
      zone_names = c(
        "Temperate", "Grassland", "Desert",
    ...
    taylor_from_spatraster_zones(
      obs = CHELSA_Trace21_1950$tasmin,
      mod = brown_1910_1989$tasmin,
      zones = koppen,
      add = TRUE,
      sig_digits = 3,
      zone_names = c(
    ...
    taylor_from_spatraster_zones(
      obs = CHELSA_Trace21_1950$tasmax,
      mod = brown_1910_1989$tasmax,
      zones = koppen,
      add = TRUE,
      sig_digits = 3,
      zone_names = c(
    ...
    taylor_from_spatraster_zones(
      obs = CHELSA_Trace21_1950$pr,
      mod = brown_1910_1989$pr,
      zones = koppen,
      add = TRUE,
      sig_digits = 6,
      zone_names = c(
...
pdf(
  file = "03_comparisons/comparisons_taylor.pdf",
  width = 8, height = 8, onefile = TRUE, bg = "white"
)
par(mai = c(0.5, 0.5, 0.5, 0.5), mar = c(0.5, 0.5, 0.5, 0.5))
{
  taylor_from_sds_monthly(
    obs_sds = agcd,
