...
for var in "${vars[@]}"; do
    ...
    files=$(ls "${base_dir}/${var}"/TraCE-Sahul_*_${var}*.nc | grep -v '_annSummary\.nc$' | sort -V)
    ...
    for f in $files; do
        ...
        cdo -L -s -w unpack -sellonlatbox,125.0,135.0,-13.0,-8.0 "$f" "$outfil"
...
# for var in "${vars[@]}"; do
#     echo -e "${GREEN}Processing $var...${RESET}"
#     files=$(ls "${base_dir}/${var}"/TraCE-Sahul_1500_1990_${var}.nc | grep -v '_annSummary\.nc$' | sort -V)
...
#         mkdir -p "$outdir"
#         outfil="$outdir/$(basename "$f")"
#         cdo -L -s -w settaxis,1500-01-01,,1mon \
#             -setcalendar,365_day \
#             -sellonlatbox,125.0,135.0,-13.0,-8.0 \
#             "$f" "$outfil"
#         echo -e "${YELLOW} Finished $(basename "$(dirname "$f")")/$(basename "$f")...${RESET}"
#     done
#     echo -e "${GREEN}Finished $var...${RESET}"
